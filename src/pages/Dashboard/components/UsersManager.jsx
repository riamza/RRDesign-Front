...
const UsersManager = () => {
  ...
  const getStatusInfo = (status) => {
    const map = {
      active: {
        color: "#22c55e",
        text: t("dashboard.usersManager.statusActive"),
        icon: CheckCircle,
      },
      pending: {
        color: "#f59e0b",
        text: t("dashboard.usersManager.statusPending"),
        icon: Clock,
      },
      suspended: {
        color: "#ef4444",
        text: t("dashboard.usersManager.statusSuspended"),
        icon: Ban,
      },
    };
  ...
  useEffect(() => {
    if (viewingUser) {
      setUserProjects([]);
      api
        .getClientProjectsByUser(viewingUser.id)
  ...
  const getActionMessage = () => {
    if (actionType === "suspend") {
      return t("dashboard.usersManager.confirmSuspendMessage", {
        name: actionUserName,
      });
    }
  ...
  return (
    <div className="manager">
      ...
      <div
        ...
        style={{ marginBottom: "2rem", display: "flex", gap: "10px" }}
      >
      ...
      {viewingUser && (
        ...
        >
          <div className="user-details">
            ...
            <div
              className="user-projects-list-mini"
                ...
                <ul style={{ listStyle: "none", padding: 0 }}>
                  {userProjects.map((p) => (
                    ...
                    >
                      ...
                      <span
                        className={`status-pill ${p.status === "Active" ? "status-active" : "status-completed"}`}
                        style={{
                          fontSize: "0.7rem",
                          padding: "2px 8px",
                          borderRadius: "12px",
                          background:
                            p.status === "Active" ? "#e0f2fe" : "#dcfce7",
                          color: p.status === "Active" ? "#0284c7" : "#166534",
                        }}
                      >
                        {p.status || "Active"}
                      </span>
